...
class BatchNormalizer:
    def __init__(self):
        self.r = 1
        self.b = 0
        self.optimizer_r = Optimizer('SGD')
...
class PoolingLayer(Layer):
    ...
    def forward(self, x):
        lx = int(x.shape[2] / self.ps)
        ly = int(x.shape[3] / self.ps)

        dim = x.shape
        bs, cn = dim[0], dim[1]

        submatrices = as_strided(x, (bs, cn, lx, ly, self.ps, self.ps), (
            dim[1] * dim[2] * dim[3] * 8, dim[2] * dim[3] * 8, dim[2] * 8 * self.ps, 8 * self.ps, dim[3] * 8, 8))

        # max pooling
        submatrices = submatrices.reshape(bs * cn * lx * ly, self.ps * self.ps)
        y = np.max(submatrices, axis=-1)
        self.address = np.reshape(np.identity(4)[np.argmax(submatrices, axis=1)], bs, cn * lx * ly)


        # test
        # test = y.reshape((bs, cn * lx * ly))
        # print(test[0].reshape(cn, lx, ly))
        # print(test[-1].reshape(cn, lx, ly))

        return y.reshape(bs, cn * lx * ly)

    def backward(self, grad_y):
        return grad_y * self.address
...
class Network:
    ...
    def testing(self, X, Y):
        return np.sum(self._forward_propagation(X, False).argmax(axis=1) == Y.argmax(axis=1)) / X.shape[0]
...
for method in optimization_methods:
    ...
    network = Network({
        0: ConvolutionLayer(1, 3, 5, 'relu', method),
        1: Reshaper(3, 28, 28),
        2: DenceLayer(3 * 28 * 28, 900, 'dropout', method),
        3: BatchNormalizer(),
        4: DenceLayer(900, 100, 'dropout', method),
        5: BatchNormalizer(),
        6: DenceLayer(100, 10, 'softmax', method),
    })
